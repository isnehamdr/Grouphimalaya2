...
const jobOpenings = [
  { id: 1, type: "Onsite", title: "Sales Executive",             description: "Lorem ipsum dolor sit amet consectetur adipisicing elit. Facilis, impedit doloribus iusto vel harum voluptatibus quam perspiciatis laboriosam omnis provident.", time: "Fulltime" },
  { id: 2, type: "Onsite", title: "Real Estate Project Manager", description: "Lorem ipsum dolor sit amet consectetur adipisicing elit. Facilis, impedit doloribus iusto vel harum voluptatibus quam perspiciatis laboriosam omnis provident.", time: "Fulltime" },
  { id: 3, type: "Onsite", title: "Finance & Banking Advisor",   description: "Lorem ipsum dolor sit amet consectetur adipisicing elit. Facilis, impedit doloribus iusto vel harum voluptatibus quam perspiciatis laboriosam omnis provident.", time: "Fulltime" },
]
  ...
  useEffect(() => {
    ...
    const initGSAP = async () => {
      ...
      ctx = gsap.context(() => {
        ...
        // Job sticky heading slides in from left
        ...
        // Job cards slide in from right, staggered
    ...
    <MainWrapper>
      ...
      <div className='w-full min-h-screen p-2 sm:p-4 overflow-x-hidden text-white'>
        ...
        <div ref={whySectionRef} className='py-12 sm:py-20 flex flex-col items-center gap-3 px-4'>
          <p
            ref={whyTitleRef}
            className='text-2xl sm:text-4xl lg:text-6xl text-center font-medium opacity-0'
          >
            Why Build Your Career With Us?
          </p>
          <p
            ref={whySubRef}
            className='text-center text-sm sm:text-base lg:text-xl max-w-xs sm:max-w-lg lg:max-w-2xl text-gray-600 opacity-0'
          >
        ...
        <div className='grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6 px-2 sm:px-4 lg:px-20'>
          {careerContents.map((item, i) => (
            <div
              ...
              className='rounded-2xl p-5 sm:p-4 lg:p-8 bg-[#121318] opacity-0'
            >
                ...
                {jobOpenings.map((item, i) => (
                  ...
                  >
                    <div className='flex flex-col gap-3 sm:gap-4'>
                      <p className='text-base sm:text-lg font-medium text-amber-700'>{item.type}</p>
                      <p className='text-2xl sm:text-3xl lg:text-4xl'>{item.title}</p>
                      <p className='text-sm sm:text-base lg:max-w-lg lg:text-lg text-gray-600'>{item.description}</p>
                      <p className='text-sm sm:text-lg'>{item.time}</p>
                    </div>
                    <div className='flex-shrink-0'>
                      <button className='bg-[#4b4640] px-4 py-2 rounded-full text-white text-sm sm:text-base'>
                        View Openings
                      </button>
                    </div>
                  </div>
