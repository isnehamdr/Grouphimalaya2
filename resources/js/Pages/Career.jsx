...
const Career = () => {
  ...
  // SEO data with consistent career keywords
  const seoData = {
    title: "Careers | Join Our Mission-Driven Team in Nepal | Himalaya Organization",
    description: "Explore career opportunities at Himalaya Organization. Join a purpose-driven workplace in Nepal offering meaningful work, growth opportunities, supportive culture, and work-life balance. Be part of empowering communities since 1991.",
    url: "https://www.himalayaorganization.com/careers",
  ...
  // Generate job openings count for meta description
  const jobCount = jobOpenings.length
  const dynamicDescription = jobCount > 0 
    ? `Join Himalaya Organization in Nepal. Currently hiring ${jobCount} position${jobCount > 1 ? 's' : ''} including ${jobOpenings.slice(0, 3).map(j => j.title).join(', ')}. Purpose-driven careers with growth opportunities since 1991.`
    : seoData.description

  return (
    <MainWrapper>
      <SEO {...seoData} description={dynamicDescription} />
          ...
          <div className='mx-auto'>
            <div className='grid grid-cols-1 lg:grid-cols-[1fr_1.4fr] gap-8 sm:gap-12 md:gap-16 items-start'>
              <div ref={jobStickyRef} className='lg:sticky lg:top-20 opacity-0'>
                ...
                {jobCount > 0 && (
                  <p className='mt-2 text-sm text-[#ae8755]'>
              ...
              <div className='flex flex-col gap-4 sm:gap-5 md:gap-6'>
                {jobOpenings.length > 0 ? jobOpenings.map((item, i) => (
                  ...
                  <div className='bg-[#121318] p-6 rounded-2xl text-gray-400'>
                    No openings are published right now. Please check back later for opportunities to join our mission-driven team.
                  </div>
                )}
              </div>
            </div>
